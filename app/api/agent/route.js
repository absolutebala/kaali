...
export async function GET(request) {
  ...
  const [{ data: waiting }, { data: live }, { data: active }, { data: agents }] = await Promise.all([
    ...
    supabaseAdmin.from('conversations')
      ...
      .order('handoff_at', { ascending: true }),
    supabaseAdmin.from('conversations')
      .select('id, visitor_type, started_at, page_url, country, city, device, org')
      .eq('tenant_id', tenant.tenantId).eq('status', 'active')
  ...
  return NextResponse.json({ waiting: waiting || [], live: live || [], active: active || [], onlineCount: agents?.length || 0 })
}
...
export async function POST(request) {
  ...
  const { data: tenantData } = await supabaseAdmin
    .from('tenants').select('ai_provider, ai_model, api_key, company, alert_email').eq('id', tenant.tenantId).single()
...
Summary:`
      ...
      // Call tenant's AI
      const apiKey = tenantData?.api_key ? decryptKey(tenantData.api_key) : null
      console.log('[Summary] provider:', tenantData?.ai_provider, 'hasKey:', !!apiKey)

      if (apiKey && tenantData?.ai_provider === 'claude') {
        const res = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model: tenantData?.ai_model || 'claude-haiku-4-5-20251001', max_tokens: 200, messages: [{ role: 'user', content: summaryPrompt }] })
        })
        const d = await res.json()
        summary = d.content?.[0]?.text || ''
      } else if (apiKey && tenantData?.ai_provider === 'openai') {
        const res = await fetch('https://api.openai.com/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
          body: JSON.stringify({ model: tenantData?.ai_model || 'gpt-4o-mini', max_tokens: 200, messages: [{ role: 'user', content: summaryPrompt }] })
        })
        const d = await res.json()
    ...
    await supabaseAdmin.from('conversations').update({
      ...
      handoff_msg: summary || 'Visitor requested to speak with a team member.',
    ...
    return NextResponse.json({ ok: true, summary: summary || 'Visitor requested to speak with a team member.' })
  }
  ...
  if (action === 'intervene') {
    ...
    const { data: convo } = await supabaseAdmin.from('conversations')
      .select('id, status').eq('id', conversationId).eq('tenant_id', tenant.tenantId).maybeSingle()
    if (!convo) return NextResponse.json({ error: 'Conversation not found' }, { status: 404 })

    // Fetch messages for summary
    const { data: msgs } = await supabaseAdmin.from('messages')
      .select('role, content, is_agent').eq('conversation_id', conversationId)
      .order('created_at', { ascending: true })

    // Generate AI summary
    let summary = 'Agent joined the conversation.'
    try {
      const transcript = (msgs || []).filter(m => !m.is_agent)
        .map(m => `${m.role === 'user' ? 'Visitor' : 'Bot'}: ${m.content}`).join('\n')
      const summaryPrompt = `Summarize this chat in 2-3 sentences for a team member who is joining:\n\n${transcript}\n\nSummary:`
      const apiKey = tenantData?.api_key ? decryptKey(tenantData.api_key) : null
      if (apiKey && tenantData?.ai_provider === 'claude') {
        const res = await fetch('https://api.anthropic.com/v1/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
          body: JSON.stringify({ model: tenantData?.ai_model || 'claude-haiku-4-5-20251001', max_tokens: 150, messages: [{ role: 'user', content: summaryPrompt }] })
        })
        const d = await res.json(); summary = d.content?.[0]?.text || summary
      } else if (apiKey && tenantData?.ai_provider === 'chatgpt') {
        const res = await fetch('https://api.openai.com/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` },
          body: JSON.stringify({ model: tenantData?.ai_model || 'gpt-4o-mini', max_tokens: 150, messages: [{ role: 'user', content: summaryPrompt }] })
        })
        const d = await res.json(); summary = d.choices?.[0]?.message?.content || summary
      }
    } catch(e) { console.error('[Intervene summary]', e.message) }

    await supabaseAdmin.from('conversations').update({
      status: 'live', agent_id: tenant.memberId || null, handoff_msg: summary,
    }).eq('id', conversationId).eq('tenant_id', tenant.tenantId)

    await supabaseAdmin.from('messages').insert({
      conversation_id: conversationId, tenant_id: tenant.tenantId,
      role: 'assistant', content: '👋 A team member has joined the chat. How can I help you today?', is_agent: true,
    })

    return NextResponse.json({ ok: true, summary, conversationId })
  }
