    ...
    <PageShell title="All Conversations">
      <div style={{ display:'grid', gridTemplateColumns:'260px 1fr', gap:14 }}>
        ...
        <div style={{ background:'#0C1220', border:'0.5px solid rgba(255,255,255,.07)', borderRadius:12, overflow:'hidden', maxHeight:560, overflowY:'auto' }}>
          {loading ? <div style={{ padding:24, color:'#9AAAC8', fontSize:13, textAlign:'center' }}>Loading…</div> :
            list.map(c => (
              <div key={c.id} onClick={()=>selectConvo(c.id)}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
                  ...
                  <span style={{ fontSize:11, color:'#7B8DB5' }}>{fmtDate(c.started_at)}</span>
                </div>
                <div style={{ fontSize:11.5, color:'#9AAAC8', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>
                  {c.tenants?.company||'Unknown'} · {c.lead_captured ? '✓ Lead' : 'No lead'}
                </div>
              </div>
            ))
          }
          {!loading && !list.length && <div style={{ padding:24, color:'#9AAAC8', fontSize:13, textAlign:'center' }}>No conversations</div>}
        </div>
        ...
        <div style={{ background:'#0C1220', border:'0.5px solid rgba(255,255,255,.07)', borderRadius:12, padding:14, maxHeight:560, overflowY:'auto', display:'flex', flexDirection:'column', gap:8 }}>
          {!selId ? (
            <div style={{ color:'#9AAAC8', fontSize:13, textAlign:'center', paddingTop:40 }}>Select a conversation to view transcript</div>
          ) : (
            <>
              {meta && (
                <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:4, paddingBottom:10, borderBottom:'0.5px solid rgba(255,255,255,.07)' }}>
                  ...
                  <span style={{ fontSize:11, color:'#7B8DB5' }}>·</span>
                  <span style={{ fontSize:11, color:'#9AAAC8' }}>{meta.visitor_type}</span>
              ...
              {msgs.map(m => (
                <div key={m.id} style={{ display:'flex', flexDirection:'column', maxWidth:'84%', alignSelf: m.role==='assistant'?'flex-start':'flex-end' }}>
                  ...
                  <div style={{ fontSize:10, color:'#7B8DB5', marginTop:3, padding:'0 3px', alignSelf: m.role==='assistant'?'flex-start':'flex-end' }}>
