...
export default function SettingsPage() {
  ...
  const [form, setForm] = useState({ botName:'', tone:'friendly', calendly:'', name:'', company:'' })
  ...
  useEffect(() => {
    if (!user) return
    setForm({ botName: user.botName||'Kaali', tone: user.tone||'friendly', calendly: user.calendly||'', name: user.name||'', company: user.company||'' })
  }, [user])

  async function save() {
    setSaving(true); setSaved(false)
    try {
      await tenantApi.update({ botName: form.botName, tone: form.tone, calendly: form.calendly, companyName: form.company })
      await refreshUser()
        ...
        <div className="card-body">
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
            ...
            <div className="form-row" style={{ marginBottom:0 }}>
              ...
              <input className="form-input" value={form.name} onChange={f('name')} placeholder="Bala Kandaswamy" />
            </div>
            <div className="form-row" style={{ marginBottom:0 }}>
      ...
      <div className="kb-card" style={{ border:'0.5px solid rgba(248,113,113,.2)' }}>
        ...
        <div className="card-body">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
            <div>
              <div style={{ fontSize:13, fontWeight:500, color:'var(--tx)', marginBottom:3 }}>Sign out</div>
              <div style={{ fontSize:12.5, color:'var(--tm)' }}>You can log back in anytime.</div>
            </div>
