...
;(function () {
  ...
  function showWelcome() {
    ...
    setTimeout(() => {
      ...
      ;[
        { em: '🚀', label: config.visitorBtn1, type: 'CLIENT'   },
        { em: '🤝', label: config.visitorBtn2,       type: 'EXISTING' },
        { em: '📈', label: config.visitorBtn3,                type: 'INVESTOR' },
        { em: '💬', label: config.visitorBtn4,                  type: 'GENERAL'  },
      ...
      const res = await fetch(config.apiUrl, {
        ...
        body: JSON.stringify({
          visitorData: {
            ...
            timezone:     window.__kaali_geo?.timezone   || Intl.DateTimeFormat().resolvedOptions().timeZone || '',
          },
          tenantId:       tenantId,
