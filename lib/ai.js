...
export function buildSystemPrompt(tenant, services = [], documents = [], trainingPairs = [], agentsOnline = false) {
  const toneMap = {
...
LEAD CAPTURE RULE — MANDATORY:
You MUST collect the visitor's name and email. This is your PRIMARY goal alongside answering questions.
- After your FIRST response, ask for their name: "By the way, what's your name?"
- Once you have their name, ask for email in your very next reply: "Thanks [name]! What's the best email to reach you?"
...
- If visitor skips giving email, ask once more naturally before moving on.
- NEVER skip lead capture. Even for short conversations, always ask for name then email.
...
export async function callAI({ tenant, messages, services, documents, trainingPairs = [], agentsOnline = false }) {
  ...
  const systemPrompt = buildSystemPrompt(tenant, services, documents, trainingPairs, agentsOnline)
